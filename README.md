...
See [`examples/101`](./examples/101) for a full working agent.
...
```bash
bun run example:101 --replay run_1781239262611
```

## Development

This repo is a Bun + Turborepo monorepo. The package lives in `packages/core`.

```bash
bun install
bun run build              # build packages/core
bun run example:101        # run the example agent (uses root .env)
```

Required env vars for the example: `GROQ_API_KEY`, `TAVILY_API_KEY`.
